...
# version 2.1
# generate the growth of cells on a grid
...
def update(data):
    ...
    for i in range(N - 1):
        ...
        for j in range(N - 1):
            # if not first j line normal, if first j line don't take into account the j-1 (first line minus one)
            if j != 0:
                # compute 8-neighbour sum and divide by 255 to know how many are full
                surr = (grid[i, j - 1] + grid[i, j + 1] + grid[i - 1, j] +
                        grid[i - 1, j - 1] + grid[i - 1, j + 1]) / 255
